// Dashboard/ViewModels/Base/ModelOperationBase.cs - Base para operações com modelos
...
public abstract partial class ModelOperationBase : ViewModelBase
{
    ...
    /// <summary>
    /// Formata números de predição
    /// </summary>
    protected string FormatPredictionNumbers(int[] numbers)
    {
        return string.Join(", ", Array.ConvertAll(numbers, n => n.ToString("D2")));
    }

    /// <summary>
    /// Calcula próximo concurso baseado no último disponível
    /// </summary>
    protected int GetNextConcurso()
    {
        if (_historicalData?.Count > 0)
        {
            var lastConcurso = _historicalData[_historicalData.Count - 1].Concurso;
            return lastConcurso + 1;
        }
        return DateTime.Now.Year * 1000; // Fallback
    }
    #endregion
